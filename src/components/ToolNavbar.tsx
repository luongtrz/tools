    ...
    <header className="sticky top-0 z-40 w-full border-b border-border/80 bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="mx-auto flex h-16 w-full max-w-[1600px] items-center gap-2 px-4 sm:px-6 lg:gap-3 lg:px-8">
        ...
        <Separator
          ...
          className="hidden h-6 2xl:block"
        />

        <nav
          className="hidden min-w-0 flex-1 items-center gap-1 2xl:flex"
...
function MobileMenu({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground focus:outline-none focus-visible:ring-1 focus-visible:ring-ring 2xl:hidden"
        aria-label={t("toolCategories")}
      >
        <svg
          viewBox="0 0 24 24"
      ...
      <DropdownMenuContent align="start" className="w-[280px] p-1 2xl:hidden">
