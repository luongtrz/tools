export const toolStyles = {
  splitLayout: "grid gap-5 md:grid-cols-2",
  panelActions: "mt-6 flex flex-wrap items-center justify-end gap-3",
  textarea:
    "block min-h-[280px] w-full resize-y rounded-xl border border-slate-200 bg-slate-50 p-4 font-mono text-sm leading-7 text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-orange-300 focus:ring-4 focus:ring-orange-100",
  input:
  ...
  qrPreview:
    "grid min-h-[340px] place-items-center rounded-xl bg-slate-50 p-5 [&_img]:h-auto [&_img]:w-full [&_img]:max-w-[320px] [&_img]:bg-white",
