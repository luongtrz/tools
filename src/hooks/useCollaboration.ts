...
export type CollaborationStatus = 'idle' | 'connecting' | 'connected' | 'offline' | 'error';
...
function createRoomId(): string {
  ...
  return Array.from({ length: 10 }, () => ROOM_ALPHABET[Math.floor(Math.random() * ROOM_ALPHABET.length)]).join('');
}
...
export function useCollaboration({ markdown, setMarkdown, name }: UseCollaborationOptions): CollaborationState {
  const [roomId, setRoomId] = useState<string | null>(readRoomFromUrl);
  const [status, setStatus] = useState<CollaborationStatus>(roomId ? 'connecting' : 'idle');
  ...
  const connect = useCallback(async (nextRoomId: string, { seed = false }: { seed?: boolean } = {}): Promise<boolean> => {
    if (activeRoomRef.current === nextRoomId && providerRef.current) return true;
    ...
    try {
      ...
      provider.awareness.setLocalStateField('user', { name: nameRef.current || 'Guest writer', color: '#f2633d' });
      ...
      if (seed && yText.length === 0) {
        doc.transact(() => yText.insert(0, markdownRef.current), 'initial-seed');
      }
  ...
  const setName = useCallback((nextName: string): void => {
    ...
    providerRef.current?.awareness.setLocalStateField('user', { name: nextName || 'Guest writer', color: '#f2633d' });
  }, []);
